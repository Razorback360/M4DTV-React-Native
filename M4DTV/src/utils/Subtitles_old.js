...
const Subtitles = ({
  ...
  useEffect(() => {
    ...
    // so we run this loop to decrease the length of the array to around 50 lines, making the filter method much faster
    // we use exponential decay to achieve that result, using the formula x * (1/2)^5 x being the length of the original subtitles array
      ...
      if (currentTime !== 0) {
        if (subtitles) {
            ...
            for (let i = 0; i < 10; i++) {
              if (subtitlesCopy.length > 5) {
                ...
                console.log(
                  ...
                  middleOfSubtiltesArray,
                  subtitles[middleOfSubtiltesArray].end,
                );
                if (currentTime <= subtitles[middleOfSubtiltesArray].end) {
                  ...
                  console.log(
                    `current Time: ${currentTime} is less than the ${middleOfSubtiltesArray}'s subtitle's end time: ${subtitles[middleOfSubtiltesArray].end}`,
                  );
                } else {
                  subtitlesCopy = subtitlesCopy.slice(middleOfSubtiltesArray);
                  console.log(
                    `current Time: ${currentTime} is more than the ${middleOfSubtiltesArray}'s subtitle's end time: ${subtitles[middleOfSubtiltesArray].end}`,
                  );
                }
          ...
          if (subtitles[0]) {
            ...
            if (
              videoTime >= currentSubtitleStart &&
  ...
  }, [videoTime, isSubtitlesSynced, subtitles, text, currentTime]);
  //console.log(text);
