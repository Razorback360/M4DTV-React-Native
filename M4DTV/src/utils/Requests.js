...
export const getMovie = async movie_id => {
  ...
  const req = await fetch(
    `https://api.themoviedb.org/3/movie/${movie_id}?api_key=${TMDB_API_KEY}&append_to_response=videos,images&include_image_language=en,null`,
  );
  const jsondata = await req.json();

  const reqAvailable = await fetch(
    `${M4D_API_URL}/movie?tmdbId=${movie_id}&id=${user_id}`,
  );
  const availablejsondata = await reqAvailable.json();

  const reqWatchlist = await fetch(
    `${M4D_API_URL}/watchlist?id=${user_id}&tmdbId=${movie_id}`,
  );
...
export const getShow = async show_id => {
  const user_id = await retrieveUser('user_id');
  const req = await fetch(
    `https://api.themoviedb.org/3/tv/${show_id}?api_key=${TMDB_API_KEY}&append_to_response=external_ids,videos,images&include_image_language=en,null`,
  );
  const jsondata = await req.json();

  const tvdb = jsondata.external_ids.tvdb_id;

  const reqTvdb = await fetch(
    `https://skyhook.sonarr.tv/v1/tvdb/shows/en/${tvdb}`,
  );
  const tvdbjsondata = await reqTvdb.json();

  const reqSonarr = await fetch(
    `${M4D_API_URL}/show?tvdbId=${tvdb}&id=${user_id}`,
  );
  ...
  const reqWatchlist = await fetch(
    `${M4D_API_URL}/watchlist?id=${user_id}&tmdbId=${show_id}&isShow=true`,
  );
  const watchlistjsondata = await reqWatchlist.json();
  ...
  const final_data = {
    ...
    logo:
      jsondata.images.logos.length > 0
        ? jsondata.images.logos[0].file_path
        : null,
...
export const getGenreMovies = async (genre, page = 1) => {
  const req = await fetch(
    `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=true&page=${page}&with_genres=${genre}&with_watch_monetization_types=flatrate`,
  );
  const jsondata = await req.json();
  return jsondata;
};
...
export const getGenreShows = async (genre, page = 1) => {
  const req = await fetch(
    `https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=true&page=${page}&with_genres=${genre}&with_watch_monetization_types=flatrate`,
  );
  const jsondata = await req.json();
  return jsondata;
};
...
export const getStreamMovie = async tmdb_id => {
  const user_id = await retrieveUser('user_id');
  const req = await fetch(
    `${M4D_API_URL}/key?media_type=movie&tmdbId=${tmdb_id}&id=${user_id}`,
    {method: 'POST'},
  );
  const jsondata = await req.json();
  return jsondata;
...
export const getStreamTV = async (tvdb_id, season, episode) => {
  const user_id = await retrieveUser('user_id');
  const req = await fetch(
    `${M4D_API_URL}/key?media_type=tv&tvdbId=${tvdb_id}&id=${user_id}&season=${season}&episode=${episode}`,
    {method: 'POST'},
  );
  const jsondata = await req.json();
  return jsondata;
...
export const addWatchlist = async (tmdb_id, isShow) => {
  const user_id = await retrieveUser('user_id');
  const req = await fetch(
    `${M4D_API_URL}/watchlist?id=${user_id}&isShow=${isShow}&tmdbId=${tmdb_id}`,
    {method: 'POST'},
  );
  const jsondata = await req.json();
