...
const Subtitles = ({currentTime, hasSeeked, textStyle, subtitlesArray}) => {
      ...
      if (currentTime !== 0) {
        if (subtitles) {
          ...
          if (!isSubtitlesSynced) {
            if (currentTime > parsedSubtitles[0].startTime) {
              ...
              for (let index = 0; index < subtitlesCopy.length; index++) {
                ...
                if (
                  ...
                  console.log(subtitle, indexOfCurrentSubtitle, 'lol');
          ...
          // now that the only subtitles remaining are ones to be played; we take the first one and read its values
          if (subtitles[0]) {
  ...
  }, [isSubtitlesSynced, subtitles, text, currentTime, parsedSubtitles]);
  //console.log(text);
