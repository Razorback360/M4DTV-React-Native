            ...
            if (item.media_type !== "person" && Object.keys(item).includes("media_type")) {
                ...
                return (<TouchableOpacity activeOpacity={0.5} onPress={() => {
                    ...
                    <Image
                        style={imagestyle}
                        source={{
                            uri: item && !item.backdrop_path.includes("tvdb") && !item.backdrop_path.includes("tmdb") ? `https://image.tmdb.org/t/p/original${item.backdrop_path}` : item && item.backdrop_path.includes("tvdb")? item.backdrop_path : item && item.backdrop_path.includes("tmdb") ? backdrop_path:"https://cdn.discordapp.com/attachments/836568944751804466/950428265514950696/th.png",
                        }}
                    />
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
